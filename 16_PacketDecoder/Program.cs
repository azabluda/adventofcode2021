Test.Cases.ForEach(DecodePackets);

void DecodePackets(string hex)
{
    var pos = 0;
    var bin = string.Concat(Convert.FromHexString(hex).Select(b => Convert.ToString(b, 2).PadLeft(8, '0')));
    var res = DecodePacket();

    Console.WriteLine($"Hex: {hex}");
    Console.WriteLine($"Part 1: {res.VersionSum}");
    Console.WriteLine($"Part 2: {res.Value}");
    Console.WriteLine();

    string ReadBits(int count)
    {
        string res = bin.Substring(pos, count);
        pos += count;
        return res;
    }

    Result DecodePacket()
    {
        long ToInt(string value) => Convert.ToInt64(value, 2);
        long version = ToInt(ReadBits(3)), type = ToInt(ReadBits(3));
        if (type == 4)
            for (string literalBits = string.Empty; ;)
            {
                bool last = ReadBits(1) == "0";
                literalBits += ReadBits(4);
                if (last) return new(version, ToInt(literalBits));
            }

        List<Result> subRes = new();
        if (ReadBits(1) == "0")
            for (long i = ToInt(ReadBits(15)) + pos; i > pos;)
                subRes.Add(DecodePacket());
        else
            for (long i = ToInt(ReadBits(11)); i-- > 0;)
                subRes.Add(DecodePacket());

        long sum = subRes.Sum(x => x.VersionSum) + version;
        long val = subRes.Select(x => x.Value).Aggregate(type switch
        {
            0 => (a, b) => a + b,
            1 => (a, b) => a * b,
            2 => (a, b) => a < b ? a : b,
            3 => (a, b) => a > b ? a : b,
            5 => (a, b) => a > b ? 1 : 0,
            6 => (a, b) => a < b ? 1 : 0,
            7 => (a, b) => a == b ? 1 : 0,
            _ => throw new NotImplementedException()
        });
        return new(sum, val);
    }
}

record Result(long VersionSum, long Value);

static class Test
{
    public static List<string> Cases { get; } = new()
    {
        "D2FE28",
        "38006F45291200",
        "EE00D40C823060",
        "8A004A801A8002F478",
        "620080001611562C8802118E34",
        "C0015000016115A2E0802F182340",
        "A0016C880162017C3686B18A3D4780",
        "C200B40A82",
        "04005AC33890",
        "880086C3E88112",
        "CE00C43D881120",
        "D8005AC2A8F0",
        "F600BC2D8F",
        "9C005AC2F8F0",
        "9C0141080250320F1802104A08",
        "",
    };
}
